...
class Monitor:
    ...
    def _handle_data_label(self, addr, line, pcb):
        if len(line.split()) !=2:
            raise ValueError("Illegal format: __data: must be followed by a value.")
        dataVal = int(line.split()[1])
        pcb.set_high_mem(dataVal+addr)
        if self._debug:
            print("__data is adding ", dataVal, " memory locations to the end.")
