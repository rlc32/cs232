...
class CPU:
    ...
    def set_mmu_registers(self, low, high):
        self._mmu.set_reloc(low)
        self._mmu.set_limit(high)
    ...
    def run_cpu(self):
        ...
        while True:
            ...
            self.parse_instruction(self._mmu.getitem(self._registers['pc']))
    ...
    def handle_mov(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.setitem(self._registers[dst[1:]], srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.setitem(eval(dst),  srcval)
    ...
    def handle_add(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.setitem(self._registers[dst[1:]], self._mmu.getitem(self._registers[dst[1:]]) + srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.setitem(eval(dst), self._mmu.getitem(eval(dst) + srcval))
